...
import { safeKvGet, getKvClient } from '@/lib/kv'
import { isEncryptionReady } from '@/lib/stripe-connection'
...
export async function GET() {
 ...
 // ── Infrastructure checks (KV, encryption) ──────────────────────────────
 const infrastructure: Record<string, 'ok' | 'failed' | 'not_configured'> = {}

 // Check KV connectivity
 try {
 const kv = getKvClient()
 if (kv) {
 await safeKvGet('__health_check__')
 infrastructure.kv = 'ok'
 } else {
 infrastructure.kv = 'not_configured'
 }
 } catch {
 infrastructure.kv = 'failed'
 }

 // Check encryption key
 const encryptionStatus = isEncryptionReady()
 infrastructure.encryption = encryptionStatus.ready ? 'ok' : 'failed'

 // Update overall status based on infrastructure
 if (infrastructure.kv === 'failed' || infrastructure.encryption === 'failed') {
 status = 'down'
 } else if (infrastructure.kv === 'not_configured') {
 status = 'degraded'
 }
