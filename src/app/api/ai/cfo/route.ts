...
import { safeKvGet, safeKvSet, getKvClient } from '@/lib/kv'
...
export async function POST(req: NextRequest) {
 ...
 if (!kv) {
 logger.error('ai/cfo', 'KV client unavailable for rate limiting')
 return NextResponse.json(
 ...
 // Set expiry on first increment (24h + buffer)
 if (count === 1) {
 await kv.expire(rateKey, 172800)
 }
 ...
 logger.error('ai/cfo', 'Request error', { error: error.message })
 return NextResponse.json({
